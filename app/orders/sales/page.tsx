...
import { Order, Stock } from '@/shared';
...
const SalesPage = () => {
    ...
    const generateOrderNumber = () => {
        const now = new Date();
        const orderNumber = `SO${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
        setValue('orderNumber', orderNumber);
    };

    const fetchOrders = async () => {
        try {
            const response = await fetch(`/api/orders?type=sales&page=${pagination.page}&limit=${pagination.limit}`);
            if (response.ok) {
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">Manage customer orders and sales</p>
                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => setValue('status', value as 'pending' | 'confirmed' | 'shipped' | 'delivered' | 'cancelled')}
                                        >
                                            <SelectTrigger>
                                                <SelectValue />
                                            </SelectTrigger>
